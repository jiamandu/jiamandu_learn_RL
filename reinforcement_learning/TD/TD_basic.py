...
def TD_Iteration(world_matrix, pi_matrix, mov_matrix, state_matrix, action_reward_matrix, q_matrix):
    for iteration in range(TD_times):
        ...
        while world_matrix[row, colum] != target_return and count != truncate_count:
            ...
            #q update
            v_t = state_matrix[row, colum]
            action, row_next, colum_next, action_reward = Choose_Action(pi_matrix, mov_matrix, row, colum, action_reward_matrix)
            v_t_next = state_matrix[row_next, colum_next]

            v_tp1 = v_t - alpha * (v_t - (action_reward + gama * v_t_next))

            state_matrix[row, colum] = v_tp1

            Calculate_Q_Matrix(q_matrix, state_matrix, mov_matrix, action_reward_matrix)

            #policy update
            if action == q_matrix[:, row, colum].argmax():
                for k in range(pi_matrix.shape[0]):
                    if k == action:
