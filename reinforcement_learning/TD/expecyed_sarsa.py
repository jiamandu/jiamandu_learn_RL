...
def TD_Iteration(world_matrix, pi_matrix, mov_matrix, state_matrix, action_reward_matrix, q_matrix):
    for iteration in tqdm(range(TD_times), desc="处理中", unit="项"):
        row = 0
        colum = 0
        count = 0
        while world_matrix[row, colum] != target_return and count != truncate_count:
            ...
            q_e = 0
            for a in range(pi_matrix.shape[0]):
                q_e += pi_matrix[a, row_next, colum_next] * q_matrix[a, row_next, colum_next]

            q_tp1 = q_t - alpha * (q_t - (action_reward + gama * q_e))
