...
def TD_Iteration(world_matrix, pi_matrix, mov_matrix, state_matrix, action_reward_matrix, q_matrix):
    for iteration in tqdm(range(TD_times), desc="Learning", unit="times"):
        row = 0
        colum = 0
        count = 0
        ...
        while world_matrix[row, colum] != target_return and count != truncate_count:
            ...
            for n in range(n_step):
                action_next, row_next, colum_next, action_reward = Choose_Action(pi_matrix, mov_matrix, row_next, colum_next,action_reward_matrix)
                if n == 0:
                    action_now = action_next
                    q_t = q_matrix[action_now, row, colum]
                td_target += mul_gama * action_reward
                mul_gama *= gama
                if world_matrix[row_next, colum_next] == target_return:
                    count = truncate_count
                    break
            action_next, _, _, _ = Choose_Action(pi_matrix, mov_matrix, row_next, colum_next,action_reward_matrix)
            td_target += mul_gama * q_matrix[action_next, row_next, colum_next]

            q_tp1 = q_t - alpha * (q_t - td_target)
